    ...
    public class ReviewRepository : IRepository<ReviewDocument>
    {
        ...
        public Task DeleteAsync(string id)
        {
            return _client.DeleteDocumentAsync(
                            UriFactory.CreateDocumentUri(_databaseName, _reviewCollectionName, id), new RequestOptions() { PartitionKey = new PartitionKey(id) });
        }

        public async Task<ReviewDocument> GetByIdAsync(string id)
        {
            return await _client.ReadDocumentAsync<ReviewDocument>(UriFactory.CreateDocumentUri(_databaseName, _reviewCollectionName, id), new RequestOptions() {PartitionKey = new PartitionKey(id)});
        }

        public Task<List<ReviewDocument>> ListAsync()
        {
            var reviews =
               from f in _client.CreateDocumentQuery<ReviewDocument>(_collectionUri, new FeedOptions(){ EnableCrossPartitionQuery = true})
               select f;
